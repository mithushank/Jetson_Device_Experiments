...
logger = [InteractiveLogger(),CSVLogger(log_folder = "mir/core50")]
...
strategy = MIR(
    ...
    eval_mb_size=50,
    batch_size_mem=1000,
    plugins = [replay_plugin, eval_plugin]
)

# start tracking using tegrastats
interval = 500 # read every 500 ms
tegrastats_process = start_tegrastats(interval=interval,output_file='tegrastats_output_mir_core50.txt')
...
total_gpu_soc_power, total_cpu_cv_power, total_power = parse_tegrastats_output('tegrastats_output_mir_core50.txt', interval)
...
csv_file_name = 'results_mir_core50.csv'
